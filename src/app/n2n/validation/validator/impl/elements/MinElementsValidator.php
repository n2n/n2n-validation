 ...
 */
namespace n2n\validation\validator\impl\elements;
...
class MinElementsValidator extends SimpleValidatorAdapter {
	...
	protected function testSingle(Validatable $validatable, ValidationContext $validationContext, MagicContext $magicContext): bool {
		$value = $this->readSafeValue($validatable, TypeConstraints::type([\Countable::class, 'array']));

		return $value === null || $this->min <= count($value);
	}


	protected function createErrorMessage(Validatable $validatable, MagicContext $magicContext): Message {
		return ValidationMessages::minElements($this->min, $this->readLabel($validatable));
	}
}